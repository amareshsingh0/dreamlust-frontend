...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const transformedResults = topResults.map(item => ({
      ...
      duration: item.duration && item.duration > 0 ? formatDuration(item.duration) : undefined,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const transformedResults = uniqueIncompleteViews.map(item => {
      ...
      duration: item.content.duration && item.content.duration > 0 ? formatDuration(item.content.duration) : undefined,
