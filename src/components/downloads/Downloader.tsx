 ...
 * Real-time download component with Socket.io integration and polling fallback
 */
...
export function Downloader({ contentId, onDownloadComplete }: DownloaderProps) {
  ...
  useEffect(() => {
    ...
    socket.on('download:completed', (data: { downloadId: string }) => {
      ...
      if (onDownloadComplete) {
        onDownloadComplete(data.downloadId);
      }
  ...
  useEffect(() => {
    ...
    if (hasActive) {
      const interval = setInterval(() => {
        loadDownloads();
      }, 3000); // Poll every 3 seconds for active downloads
  ...
  const handleStartDownload = async () => {
    if (!contentId || !user) {
      toast.error('Please select content to download');
      return;
    }
  ...
  const handleDelete = async (downloadId: string) => {
    try {
      await api.downloads.delete(downloadId);
                      ...
                      <div className="mt-2 space-y-2">
                        ...
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-muted-foreground">
                            {download.progress}% • {formatFileSize(download.fileSize)} • {download.quality}
                          </span>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        <span className="text-xs text-muted-foreground">
                          Expires: {formatDistanceToNow(new Date(download.expiresAt), { addSuffix: true })} • {download.quality}
                        </span>
                        {getStatusBadge(download.status)}
                      </div>
                    </div>
